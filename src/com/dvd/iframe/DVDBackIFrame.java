...
public class DVDBackIFrame extends JInternalFrame {
	...
	private String[] columnNames = { "DVD名称", "DVD条形码","DVD类别","租借者姓名","租借者条形码","借书时间","归还时间" };
	DefaultTableModel model = new DefaultTableModel();
	SimpleDateFormat myfmt=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
	private String dvdISBNs=null;
	private String readerISBNs=null;
	private int id;
	
	public final void add() {
		readerISBNs=readerISBN.getText().trim();
		List list=Dao.selectDVDBack(readerISBNs);
		for(int i=0;i<list.size();i++){
			Borrow back=(Borrow)list.get(i);
		id=	back.getId();
			String str[] = new String[7];
			str[0] =back.getDVDName();
			str[1] =back.getDVDISBN();
	...
	class TableListener extends MouseAdapter {
		public void mouseClicked(final MouseEvent e) {
			...
			List list =Dao.selectDVDTypeFk(table.getValueAt(selRow, 2).toString().trim());
			for(int i=0;i<list.size();i++){
				DVDType dvdtype=(DVDType)list.get(i);
				fk=dvdtype.getFk();
				days1=dvdtype.getDays();
			}
			...
			dvdISBNs=table.getValueAt(selRow, 1).toString().trim();
		}
	}
	class BookBackActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			// TODO 自动生成方法存根
			if(readerISBNs==null){
				JOptionPane.showMessageDialog(null, "请输入租借者编号！");
				return;
			}
			System.out.println(dvdISBNs==null);

			if(table.getSelectedRow()==-1){
				JOptionPane.showMessageDialog(null, "请选择所要归还的DVD！");
				return;	
			}

		
			int i=Dao.UpdateDVDBack(dvdISBNs, readerISBNs,id);
			
			System.out.print(i);
			 if(i==1){	
				int selectedRow = table.getSelectedRow();
				model.removeRow(selectedRow);
				JOptionPane.showMessageDialog(null, "还DVD操作完成！");		
			}
		}
	}
